...
if (nexacro._Browser == "Runtime") {
	...
	if (!nexacro.Element) {
		...
		_pInputElement._on_sys_keypress = function () {
			var handle = this.handle;
			if (handle) {
				if (this.maxlength > 0) {
					var value = this._getInputValue();
		...
		nexacro._PluginObject = function (handle) {
			this.handle = handle;
		...
		__pPluginObject.getProperty = function () {
			if (this.handle) {
				...
				if (value != null && typeof (value) == "object") {
					var pobject = new nexacro._PluginObject(value);
					return pobject;
				}
				return value;
			}
		...
		__pWebBrowserPluginElement._pluginCallMethod = function (args) {
			if (nexacro.OS == "Android" && nexacro._Browser == "Runtime") {
				var str = args[0];
			...
			if (comp) {
				if (evt_id == "PlayStateChange") {
					...
					if (typeof arg0 == "object") {
						var mediaobj = new nexacro._PluginObject(arg0);
						if (mediaobj) {
