...
if (!nexacro.Component) {
	...
	_pComponent.set_tooltiptype = function (v) {
		var tooltiptype_enum = ["default", "hover", "inplace"];
	...
	_pComponent.set_scrollbarbarminsize = function (scrollbarbarminsize) {
		...
		if (scrollbarbarminsize != scrollbarbarminsize) {
			return;
		}
	...
	_pComponent.set_scrollbardecbuttonsize = function (scrollbardecbuttonsize) {
		...
		if (scrollbardecbuttonsize != scrollbardecbuttonsize) {
			return;
		}
	...
	_pComponent.set_scrollbarbaroutsize = function (scrollbarbaroutsize) {
		...
		if (scrollbarbaroutsize != scrollbarbaroutsize) {
			return;
		}
	...
	_pComponent.set_scrollbarincbuttonsize = function (scrollbarincbuttonsize) {
		...
		if (scrollbarincbuttonsize != scrollbarincbuttonsize) {
			return;
		}
	...
	_pComponent.set_scrollbarsize = function (scrollbarsize) {
		...
		if (scrollbarsize != scrollbarsize) {
			return;
		}
	...
	_pComponent.set_scrollindicatorsize = function (scrollindicatorsize) {
		...
		if (scrollindicatorsize != scrollindicatorsize) {
			return;
		}
	...
	_pComponent.set_scrollbartrackbarsize = function (scrollbartrackbarsize) {
		...
		if (scrollbartrackbarsize != scrollbartrackbarsize) {
			return;
		}
		...
		if (fittocontents == "width" || fittocontents == "both") {
			...
			if (info && (obj = info.width)) {
				if (form && form._is_created_contents) {
					...
					this._width = this._convToPixel(obj.distance, comp._getClientWidth());
				}
			}
			else {
				if (this.width != null) {
		...
		if (fittocontents == "height" || fittocontents == "both") {
			...
			if (info && (obj = info.height)) {
				if (form && form._is_created_contents) {
					...
					this._height = this._convToPixel(obj.distance, comp._getClientHeight());
				}
			}
			else {
				if (this.height != null) {
	...
	_pComponent._getPopupContainerCSSSelector = function () {
		...
		if (popupcontrol) {
			...
			if (parent._is_subcontrol) {
				cssselector_str = "popup" + parent.on_get_css_assumedtypename() + this.cssclass + this.on_getIDCSSSelector() + ",";
			}

			while (parent && parent._is_subcontrol) {
				parentid = parent.on_getIDCSSSelector() + parentid;
				parent = parent.parent;
			}

			cssselector_str += "popup" + comp_type_name + parentid + this.cssclass + this.on_getIDCSSSelector();
